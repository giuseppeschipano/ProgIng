...
public class PrenotazioneService {
    ...
    public void effettuaPrenotazione(Prenotazione p) {
        try (Connection conn = DBConnectionSingleton.getConnection()) {
            ...
            int postiDisponibili = trattaRepo.getPostiDisponibili(conn, p.getId_tratta());
            if (postiDisponibili <= 0) {
                System.out.println("Treno pieno. Impossibile prenotare.");
                return;
            }

            prenotazioneRepo.aggiungiPrenotazione(p, conn);
            trattaRepo.decrementaPostiDisponibili(conn, p.getId_tratta(), 1);
    ...
    public void confermaPrenotazione(Prenotazione p, Biglietto b) {
        try (Connection conn = DBConnectionSingleton.getConnection()) {
            ...
            prenotazioneRepo.rimuoviPrenotazione(p.getId_Prenotazione(), conn);
    ...
    public void annullaPrenotazione(Prenotazione p) {
        try (Connection conn = DBConnectionSingleton.getConnection()) {
            ...
            prenotazioneRepo.rimuoviPrenotazione(p.getId_Prenotazione(), conn);
            trattaRepo.incrementaPostiDisponibili(conn, p.getId_tratta(), 1);
